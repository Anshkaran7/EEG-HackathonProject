...
// import giraffeImage from './images/girafe.png';
// import zebraImage from './images/zebra.png';
...
function MemoryGame() {
  ...
  <div
    key={index}
    className={`bg-blue-200 rounded-md h-32 flex justify-center items-center cursor-pointer transition-transform transform hover:scale-105 ${
  ...
  >
    {flippedIndexes.includes(index) || matchedPairs.includes(animal.name) ? (
      <img src={animal.image} alt={animal.name} className="w-32 h-32 object-cover" />
    ) : (
      <span>?</span>
    )}
  </div>
